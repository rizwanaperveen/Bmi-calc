...
export default function BmiCalculator() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 dark:bg-gray-900">
      {/* Center the BMI calculator card within the screen */}
      <Card className="w-full max-w-md mx-auto bg-red-200">
        <CardHeader>
          {/* Header with title and description */}
          <CardTitle className="text-2xl font-bold text-purple-950 text-center">BMI Calculator</CardTitle>
          <CardDescription className="text-slate-800 text-center">
            Enter your height and weight to calculate your BMI.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Input for height */}
          <div className="grid gap-2 text-indigo-900">
          ...
          <Button className=" bg-purple-700 text-white w-full hover:bg-indigo-900" onClick={calculateBmi}>Calculate</Button>
          {/* Display error message if any */}
          {error && <div className="text-red-500 text-center">{error}</div>}
          {/* Display BMI result if available */}
          {result && (
            <div className="grid gap-2 text-indigo-700">
